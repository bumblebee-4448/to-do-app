...
Tài liệu này mô tả chi tiết yêu cầu bài toán, kiến trúc hệ thống, thiết kế API, cấu trúc thư mục, kế hoạch triển khai và kiểm thử cho ứng dụng **Quản lý công việc (Todo List)** sử dụng **ReactJS + TypeScript** (Frontend) và **NodeJS + ExpressJS + TypeScript** (Backend).
...
*   **Styling & UI Library:** TailwindCSS 4 (xây dựng giao diện nhanh, linh hoạt) kết hợp với các component local theo phong cách **shadcn/ui**.
...
*   **Query Parameters:**
    ...
    *   `sortBy`: Sắp xếp theo trường mong muốn, ví dụ `createdAt` hoặc `dueDate` (mặc định: `createdAt`)
...
*   **Mô tả:** Cập nhật một hoặc một vài trường của Todo (ví dụ: chỉ cập nhật trạng thái `status` hoặc ngày hết hạn `dueDate`). Phục vụ đắc lực cho tính năng **Optimistic Updates**.
*   **HTTP Method:** `PATCH`
...
```text
to-do-list/
├── client/                 # ReactJS + TypeScript Frontend (Feature-based)
│   ├── public/
│   ├── src/
...
│   │   │       ├── types.ts # Types/domain contract của Todo feature
│   │   │       └── index.ts # Entry point tùy chọn xuất bản các components/hooks cần dùng ngoài feature
...
## 8. Tiến Độ & Kế Hoạch Triển Khai Chức Năng (Roadmap)

### 8.1. Trạng Thái Hiện Tại
*   **Backend (NodeJS + ExpressJS + MongoDB)**: Đã hoàn thành 100% việc cài đặt các API CRUD, các tính năng tìm kiếm, lọc, phân trang và validate dữ liệu ở server-side. Hệ thống unit test đạt tỷ lệ bao phủ đầy đủ (9/9 pass).
*   **Giao Diện (Frontend UI/Layout)**: Đã hoàn thành việc cấu hình giao diện chính bao gồm Kanban Board, bộ lọc, form thêm/sửa, thanh sidebar điều hướng và chế độ sáng/tối tự động lưu trữ thông qua Zustand Store.
*   **Chức năng Frontend**: Cơ bản đã kết nối thành công các React Query Mutations (thêm, sửa, xóa, hoàn thành). Tuy nhiên, bộ chuyển đổi chế độ xem (Bảng, Công việc, Biểu đồ, Danh sách) chưa được kích hoạt và các trang view tương ứng chưa được liên kết hoàn chỉnh.

### 8.2. Lộ Trình Triển Khai Tiếp Theo (Thứ Tự Ưu Tiên)

Dưới đây là các bước và chức năng cần tiếp tục triển khai theo thứ tự logic:

1.  **Sửa lỗi kiểm thử frontend hiện tại**:
    *   Cập nhật lại test case kiểm tra tiêu đề trong [App.test.tsx](file:///d:/PROJECT/to-do-list/client/src/__tests__/App.test.tsx) để thay thế chuỗi khớp tiêu đề từ `design for landing page` sang `Danh sách việc cần làm` nhằm khớp với giao diện thực tế.
2.  **Kích hoạt bộ chuyển đổi chế độ xem (View Switcher)**:
    *   Thêm trạng thái `view` (`'board' | 'tasks' | 'chart' | 'list'`) trong component [TodoDashboard.tsx](file:///d:/PROJECT/to-do-list/client/src/features/todos/components/TodoDashboard.tsx) để phản hồi khi người dùng chuyển đổi qua lại giữa các tab hiển thị.
3.  **Tích hợp chế độ xem "Danh sách" (List View)**:
    *   Tích hợp các component có sẵn là [TodoList.tsx](file:///d:/PROJECT/to-do-list/client/src/features/todos/components/TodoList.tsx) và [TodoItem.tsx](file:///d:/PROJECT/to-do-list/client/src/features/todos/components/TodoItem.tsx) vào tab "Danh sách" để hiển thị danh sách phẳng, hỗ trợ tải thêm dữ liệu bằng cách gọi API phân trang.
4.  **Triển khai chế độ xem "Công việc" (Tasks View)**:
    *   Xây dựng giao diện gom nhóm công việc trực quan (ví dụ: gom nhóm theo Mức độ ưu tiên Low/Medium/High hoặc trạng thái thực hiện) để người dùng quản lý công việc có cấu trúc tốt hơn.
5.  **Triển khai chế độ xem "Biểu đồ" (Chart View)**:
    *   Xây dựng các thẻ thông tin thống kê tổng quan (Kpi Cards): Tổng số task, task đang chờ, task đã hoàn thành và task quá hạn.
    *   Tạo biểu đồ trực quan (như biểu đồ hình tròn biểu thị phân bố độ ưu tiên Low/Medium/High và tỷ lệ phần trăm hoàn thành công việc) bằng SVG kết hợp CSS thuần.

---

## 9. Kế Hoạch Viết Unit Test
...
## 10. Hướng Dẫn Chạy Trực Tiếp Trên Máy Local (Không Dùng Docker)
...
*   Cài đặt **Node.js** phiên bản mới nhất phù hợp với Vite/TypeScript hiện tại (khuyên dùng từ `v24.x` trở lên).
...
4.  Khởi chạy Backend Server ở chế độ nhà phát triển (tự động tải lại code TypeScript bằng `tsx watch`):
    ```bash
    npm run dev
    ```
...
Để xem database local trong MongoDB Compass, dùng URI:

```text
mongodb://127.0.0.1:27017/todo-db?directConnection=true
```

Nếu muốn chạy demo local không có MongoDB thật, đặt `ALLOW_IN_MEMORY_DB=true` ngoài môi trường production. Database memory chỉ là tạm thời và sẽ bị xóa khi process backend dừng.

### Production Runtime Rules

*   `MONGO_URI` là bắt buộc khi `NODE_ENV=production`.
*   Không dùng in-memory MongoDB trong production.
*   Đặt `CLIENT_ORIGIN` đúng origin frontend đã deploy. Không dùng wildcard CORS cho production.
*   `ALLOW_IN_MEMORY_DB=true` chỉ dành cho local development/demo.
...
*   **Kiểm tra kiểu TypeScript cho Backend:**
    ```bash
    cd server
    npm run typecheck
    ```
*   **Chạy Unit Test cho Backend (Express API):**
    ...
    npm test
    ```
*   **Kiểm tra kiểu TypeScript cho Frontend:**
    ```bash
    cd client
    npm run typecheck
    ```
*   **Chạy Unit Test cho Frontend (ReactJS + TypeScript):**
    ```bash
    cd client
    npm test
    ```
